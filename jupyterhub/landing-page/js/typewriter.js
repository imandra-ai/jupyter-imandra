/*
 * Extanded by AI Team, original script:
 * https://github.com/tameemsafi/typewriterjs by Tameem Safi 
 * MIT license
*/
...
(function() {
	...
	TypewriterPrototype.typeWords = function(string) {
		if(!string || typeof string != 'string') {
			return console.error('Please enter a string as the paramater.');
		}

		var string_chars = this._getWords(string);
	...
	//e: clear terminal screen trigger
	TypewriterPrototype.clearAll= function() {
		this._addToEventQue([this._deleteChars, ['clear']]);
		return this;
	}; //e: out
	...
	//e pulse effect
	TypewriterPrototype.pulseEffect = function(pulseClass){
		this._addToEventQue([this._pulseEffect, [pulseClass]]);
		return this;
	};//e out
	...
	TypewriterPrototype._deletingCharAnimationFrame = function() {
		...
		//e: clear terminal screen method
		if(char_amount_to_delete == 'clear') {

 			self._settings.charAmountToDelete = false;
 			self._settings.currentTypedCharacters = [];
 			current_typed_char_ids = [];

			var emptyMe = document.getElementsByClassName('typewriter-wrapper');
			while (emptyMe[0].firstChild) {
				emptyMe[0].removeChild(emptyMe[0].firstChild);
			}
	...
	// e: split strings into words 
	TypewriterPrototype._getWords = function(string) {
		if(typeof string !== 'string') {
			return false;
			} 

			var WordsArray = string.split(" ")
			var newWordsArray = WordsArray.map(function(word) {
					//console.log(word);	
					return word + " ";
				});	
	...
	TypewriterPrototype._cursorAnimationFrame = function() {
		if(!this._settings.cursorAnimationPaused) {
			...
			//console.log('Blinkspeed: ' + this.options.blinkSpeed);
	...
	TypewriterPrototype._randomID = function() {
    	...
    	for( var i=0; i < this._randomInteger(2, 8); i++ ) {
